...
# read current time ： 'YYYYMMDD_hhmmss'
...
def start_strat(strat):
    if strat.dailyExitTime is not None:
        ...
        strat.timeLog("End time ：%s, now quit......." % strat.dailyExitTime)
    else:
        while True:
            try:
                strat.go()
